"""
This module contains the RecommendationEngine class that is
...
class RecommendationEngine:
    """
    This class is used to recommend products based on the user's preferences
    and the similarity of the products' attributes.
    """
    ...
    def recommendation_function(self,
                                ...
                                n_recommendations=5):
        """
        Recommends products based on the user's preferences and the
        similarity of the products' attributes.
        """

        category_df = self.df[
            self.df['secondary_category'].str.lower() == secondary_category.lower()
            ]
        filtered_df = category_df[
            category_df['Principal_Ingredient'].str.lower() == ingredient_preference.lower()
            ]

        # 2. Further filter by user attributes.

        filtered_df = filtered_df[
            filtered_df['skin_tone'].str.lower() == skintone.lower()
            ]
        filtered_df = filtered_df[
            filtered_df['skin_type'].str.lower() == skintype.lower()
            ]
        ...
        filtered_df['composite_text'] = (filtered_df['ingredients_cleaned'] +
                                         " " +
                                         filtered_df['skin_tone'].fillna('') +
                                         " " +
                                         filtered_df['skin_type'].fillna('') +
                                         " " +
                                         filtered_df['Principal_Ingredient'].fillna(''))
        ...
        # Get indices of the filtered products (assumes df and tfidf_matrix
        # are aligned)
        valid_indices = filtered_df.index

        # Compute cosine similarity scores between the query
        # and the filtered products
        similarity_scores = cosine_similarity(input_vector,
                                              tfidf_matrix[valid_indices]).flatten()
