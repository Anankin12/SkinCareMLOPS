...
class DataProcessing:
    ...
    def apply_processing(self):
        ...
        """
        self.df['Processed_Ingredients'] = self.df['Ingredients'].apply(self.process_ingredients)
        self.df['ingredients_text'] = self.df['Processed_Ingredients'].apply(self.ingredients_to_string)
    ...
    def water_or_silicone(self):
        ...
        """
        self.df['water_based'] = self.df['Processed_Ingredients'].apply(lambda ingredients: 
                                                                        any(keyword in ingredients for keyword in self.water_keywords))
        
        self.df['silicone_based'] = self.df['Processed_Ingredients'].apply(lambda ingredients: 
                                                                           any(keyword in ingredients for keyword in self.silicone_keywords))
    ...
    def dropping_no_ingredients(self) -> pd.DataFrame: 
        ...
        placeholder_mask = self.df['Ingredients'].str.contains('visit the', case=False, na=False)
        no_info_mask = self.df['Ingredients'].str.contains('no info', case=False, na=False)
